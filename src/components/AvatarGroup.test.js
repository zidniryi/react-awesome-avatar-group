...
describe('AvatarGroup', () => {
    ...
    it('should render avatars with correct size for xs', () => {
        const { getAllByTestId } = render(<AvatarGroup names={names} size="xs" />);
        const avatars = getAllByTestId('avatar-initial-text');
        expect(avatars[0]).toHaveStyle({ width: '24px', height: '24px', fontSize: '12px' });
    });
